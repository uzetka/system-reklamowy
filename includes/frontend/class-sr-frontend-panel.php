 ...
 * sr_front_get_view_title() z Code Snippets (Front Panel).
 *
 * Na tym etapie:
 * - "Kontrahenci" obsługuje SR_Frontend_Kontrahenci (lista + formularz),
 * - "Zlecenia RADIO":
 *      - lista       → SR_Frontend_Zlecenia_Radio::render_list(),
 *      - dodawanie   → SR_Frontend_Zlecenia_Radio::render_add(),
 *      - edycja      → SR_Frontend_Zlecenia_Radio::render_edit(),
 *   z fallbackami na funkcje snippetowe:
 *      - sr_front_render_zlecenia_radio_list()
 *      - sr_front_render_zlecenia_radio_add()
 *      - sr_front_render_zlecenia_radio_edit()
 * - "Plan emisji", "Zlecenia TV", "Grafik RADIO", "Ustawienia" działają jak wcześniej.
 */
...
class SR_Frontend_Panel {
     ...
     *
     * @param string $view Klucz widoku (dashboard, konrtrahenci, zlecenia-radio, itp.).
     */
    public function render_view( string $view ): void {
        ...
        switch ( $view ) {
             ...
             * SR_Frontend_Zlecenia_Radio::render_edit() z fallbackiem.
             */
            case 'zlecenia-radio-edit':
                $id = isset( $_GET['id'] ) ? absint( $_GET['id'] ) : 0;
                ...
                if ( class_exists( 'SR_Frontend_Zlecenia_Radio' ) ) {
                    $radio = new SR_Frontend_Zlecenia_Radio();
                    $radio->render_edit( $id );

                } elseif ( function_exists( 'sr_front_render_zlecenia_radio_edit' ) ) {
                    sr_front_render_zlecenia_radio_edit();
