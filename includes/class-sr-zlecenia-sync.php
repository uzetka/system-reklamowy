...
class SR_Zlecenia_Sync {
    ...
    // Jeżeli zlecenie w koszu – oznacz jako "cancelled" w SQL.
    if ( $post->post_status === 'trash' ) {
        $this->mark_cancelled( $post_id );
        return;
    }

    // 🔥 FIX: nie synchronizujemy, jeśli meta nie są jeszcze kompletne.
    // Zapobiega tworzeniu pustych rekordów w wp_sr_zlecenia
    // przy tworzeniu zleceń z frontu (CPT dopiero powstaje).
    $required_meta = [
        'kontrahent_id',
        'nazwa_reklamy',
        'dlugosc_spotu',
    ...
    foreach ( $required_meta as $m ) {
        $v = get_post_meta( $post_id, $m, true );
        if ( $v === '' || $v === null ) {
            return; // zlecenie jest jeszcze "gołe" → nie syncujemy
        }
    }
